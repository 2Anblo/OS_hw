...
outfile << "lpMaximumApplicationAddress"<<'\t'<< std :: hex << std :: setw(16) << (DWORD64) 
info.lpMaximumApplicationAddress << endl;
outfile<<"lpminimemApplicationAddress"<<'\t'<< std :: hex << std :: setw(16) << (DWORD64)  info.lpMinimumApplicationAddress << endl;
...
#pragma warning(disable:4996)
	...
	// operation temp_op;
	// fread(&temp_op, sizeof(operation), 1, file);
	operation op;
	SYSTEM_INFO info;
	DWORD64 temp;
	GetSystemInfo(&info);
...
traceArray[index].start = VirtualAlloc(NULL, op.block * info.dwPageSize, MEM_RESERVE, PAGE_NOACCESS);
			...
			cout << "starting address:" <<  std::setw(16) << std::setfill('0') << std::hex 
     << (DWORD64)traceArray[index - 1].start << '\t'
				<< std::dec << "size:" << traceArray[index - 1].size << endl;

			break;
		}
		case 1:		//MEM_TOP_DOWN提交一个区域
		{
			cout << "commit now" << endl;

traceArray[index].start=VirtualAlloc(traceArray[index].start, traceArray[index].size, MEM_COMMIT | MEM_TOP_DOWN, temp);

			index++;
cout << "starting address:" <<  std::setw(16) << std::setfill('0') << std::hex 
     << (DWORD64) traceArray[index - 1].start << '\t' << std::dec << "size:" << traceArray[index - 1].size << endl;
		...
		case 2:		//锁一个区域
		{
			cout << "lock now" << endl;
cout << "starting address:" <<  std::setw(16) << std::setfill('0') << std::hex 
     << (DWORD64) traceArray[index].start << '\t'<< std::dec << "size:" << traceArray[index].size << endl;
...
cout << "start address:" <<  std::setw(16) << std::setfill('0') << std::hex 
     << (DWORD64) traceArray[index].start << '\t' << std::dec<< "size:" << traceArray[index].size << endl;
...
MEM_DECOMMIT))
	...
	cout << "starting address:"<<  std::setw(16) << std::setfill('0') << std::hex 
     << (DWORD64) traceArray[index].start << '\t' << std::dec<< "size:" << traceArray[index].size << endl;
			...
			if (!VirtualFree(traceArray[index++].start, 0, MEM_RESET))
				cout << GetLastError() << endl;
			break;
		}
		default:
