...
{
	//互斥变量
	HANDLE h_Mutex; //对readcount值修改做的互斥变量
	...
	{
		//第一个读者，等待资源
		EnterCriticalSection(&RP_Write);//对读写临界区的互斥变量
	}
...
while (inFile >> thread_info[n_thread].serial >> thread_info[n_thread].entity >> thread_info[n_thread].delay>> thread_info[n_thread].persist) {
    inFile.get(); // 处理行尾的换行符
    n_thread++;   // 计数器递增
}
...
void WP_ReaderThread(void *p)
{
	...
	HANDLE h_Mutex2;//对readcount值修改做的互斥变量
...
void WP_WriterThread(void *p)
{
	...
	//互斥对象
	HANDLE h_Mutex3;//对writecount值修改做的互斥变量
